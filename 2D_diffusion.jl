...
# Domain definitions
Lx::Float32 = 4 # m (plate length in x)
Ly::Float32 = 4 # m (plate length in y)
t::Float32 = 0.1 # m (plate thickness)

# Discretization
Nx::Int16 = 100 # Number of cells
Ny::Int16 = 100 # Number of cells
dx = Lx / Nx
dy = Ly / Ny
...
# Defining B.C.
S̄::Float32 = 1000 # W/m³
T_N::Float32 = 250 # °C at north
T_S::Float32 = 150 # °C at south
T_E::Float32 = 200 # °C at east
T_W::Float32 = 100 # °C at west
...
function main()
    ...
    for i in 1:Nx*Ny
        ...
        elseif cells[i].N == "Dirichlet" 
            a_n = 0
            Sp_n = -2 * Dy * Ay
            Su_n = cells[i].BC_value[1] * 2 * Dy * Ay 
        ...
        elseif cells[i].S == "Dirichlet"
            a_s = 0
            Sp_s = -2 * Dy * Ay
            Su_s = cells[i].BC_value[2] * 2 * Dy * Ay 
        elseif cells[i].N == "Neumann"
            a_s = 0
            Sp_s = 0
        ...
        elseif cells[i].E == "Dirichlet" 
            a_e = 0
            Sp_e = -2 * Dx * Ax
            Su_e = cells[i].BC_value[3] * 2 * Dx * Ax
        elseif cells[i].N == "Neumann"
            a_e = 0
            Sp_e = 0
        ...
        elseif cells[i].W == "Dirichlet" 
            a_w = 0
            Sp_w = -2 * Dx * Ax
            Su_w = cells[i].BC_value[4] * 2 * Dx * Ax
        elseif cells[i].N == "Neumann"
            a_w = 0
            Sp_w = 0
    ...
    heatmap(Xp, Yp, T', label="Temperature", xlabel="X [m]", ylabel="Y [m]", title="Temperature Distribution", c=:viridis, yflip=true)
end
